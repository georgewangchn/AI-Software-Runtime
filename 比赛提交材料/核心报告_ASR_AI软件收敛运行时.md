...
TesterAgent  ──→ opencode run --dir <sandbox> <prompt>  （无状态，每轮重新生成/更新测试）
AnalyzerAgent──→ opencode run --dir <sandbox> <prompt>  （无状态，输出纯文本分析）
```
...
| Tester | **无状态**，每轮新建沙盒 | 防止错误污染，保证每轮测试从干净状态出发 |
| Analyzer | **无状态**，每轮新建沙盒 | 防止分析偏差累积，保证每轮独立判断 |

**变更检测**（v1.2）：不再使用 git diff。Controller 通过对比 rollback 快照（修复前后的文件内容）使用 Python `difflib` 标准库生成 unified diff。消除了 git 依赖和对 Docker 环境的权限要求。
...
Analyzier 输出为纯文本（v1.2 去除 YAML 格式），直接传递给 Builder 作为修复依据。通过正则匹配 `[HIGH]`/`[CRITICAL]` 标记计算严重度计数，匹配 `ALL CLEAR` 判断规格一致性。

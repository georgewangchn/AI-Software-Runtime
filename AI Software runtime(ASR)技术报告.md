...
| v1.4 | 2026-05-28 | **系统同步**：统一 Agent prompt 为流水线模型（开发专家→测试专家→验收专家）；Tester 每轮都生成/更新测试用例；Analyzer 改为 plain text 输出；去除 git diff 依赖改用 snapshot 对比；运行时目录配置对齐 |
| v1.5 | 2026-05-29 | **运行时升级**：集成 oh-my-openagent 多智能体编排，移除 CI 模式抑制；修复非 UTF-8 文件解码崩溃；统一 Python 3.12.9 运行环境 |
...
│  内置 TODO 强制执行 + Ralph 循环（持续工作直到 100% 完成）         │
│  opencode run --format json --dir <project> [--session ID]        │
│  非 CI 模式（保留完整 agent 编排能力）                              │
└──────────────────────────────────────────────────────────────────┘
...
| **AnalyzerAgent** | `agents/analyzer.py` | 对比设计文档与代码，输出纯文本偏差分析 | 验收专家：查代码是否符合设计 |
| **OpenCode Backend** | `agents/opencode_backend.py` | 子进程调用 OpenCode CLI，解析 JSON 流，不含 git | 手：连接 AI 大脑和文件系统 |
| **EventStore** | `events/store.py` | 文件化事件存储、A2A 通信、补丁持久化 | 黑匣子：记录一切，可回放 |
| **DAGExecutor** | `dag/executor.py` | 大任务拆解、并行执行收敛循环 | 并行调度器 |
| **Config System** | `config/models.py` | Pydantic v2 配置模型，含完整运行时目录配置 | 配置中心 |
...
| Phase 5: Autonomous Engineering Runtime | 🔮 规划中 | ASR 成为 OpenCode 的自治工程运行时层 |
...
每轮迭代：
  ...
  ③ 退化检测（关键安全网）
     if 修复后失败数 > 修复前失败数 × 1.5:
         自动回滚所有文件到修复前状态
         删除修复中新增的文件

  ④ ANALYZING（语义裁决，每轮都执行）
     对比 DESIGN.md 与代码实现
     即使测试失败也执行——分析失败原因和设计偏差
     → 测试通过 + 分析对齐 → CONVERGED（收敛成功！）
...
> 为什么是 1.5 倍而不是 1 倍？因为修复过程中可能引入新的测试用例（Tester 每轮都补充测试），新的测试失败数自然会略增。1.5 倍阈值允许"合理的增长"，但防止"修坏了"的退化。

**一句话：改坏了就撤回去，绝不让你越改越烂。**

```python
# asr/controller/convergence.py — 退化检测与回滚
if before_count > 0 and after_count > before_count * 1.5 and self._rollback_entries:
    ...
    for f in self._project_dir.rglob("*"):
        if f.is_dir() or any(p in str(f) for p in ("__pycache__", ".asr_sandbox", ".git")):
            continue
        rel = str(f.relative_to(self._project_dir))
        if rel not in snapshotted:
            f.unlink(missing_ok=True)  # 删除新增文件
```
...
```
BuilderAgent ──→ [OMO] IntentGate→Prometheus→Metis→Momus→Atlas→Sisyphus ──→ opencode run
TesterAgent  ──→ [OMO] 多智能体编排 ──→ opencode run
AnalyzerAgent──→ [OMO] 多智能体编排 ──→ opencode run
```

oh-my-openagent 为每个 Agent 提供完整的编排流水线：

| 阶段 | 组件 | 职责 |
|------|------|------|
| **意图分类** | IntentGate | 识别任务类型，路由到最合适的 Agent |
| **规划** | Prometheus + Metis | 访谈式规划 + 缺口分析，确保设计理解完整 |
| **审查** | Momus | 执行前审查，不通过则驳回重做 |
| **分发执行** | Atlas → Sisyphus | 按 TODO 列表分发任务，持续工作直到 100% 完成（Ralph 循环） |
| **编辑保证** | Hash-anchored edits | 每次编辑前校验内容哈希，编辑成功率 68.3% |

**三 Agent 会话管理**：

| Agent | 会话管理 | 编排模式 | 原因 |
|-------|---------|---------|------|
| Builder | **有状态**，跨迭代保持 `session_id` | Sisyphus（Ralph 循环 + TODO 强制） | 需"记住"修改历史；OMO 保证"做完了再停" |
| Tester | **无状态**，每轮新建沙盒 | 多智能体编排 | 防止错误污染；OMO 保证测试生成完整性 |
| Analyzer | **无状态**，每轮新建沙盒 | 多智能体编排 | 防止偏差累积；OMO 保证分析深度 |

Controller 通过对比 rollback 快照（修复前后的文件内容）使用 Python `difflib` 标准库生成 unified diff 进行变更检测。
...
# 2. 在 Sandbox 中让 OpenCode 检查已有测试、修复问题、补充新测试
# 3. 验证 test_*.py 文件已生成（未生成则报 TestErrorEvent）
# 4. 执行 pytest
# 5. 将生成的测试文件回写到项目 tests/ 目录
# 6. 清理 Sandbox
```

**意义**：
- OpenCode 可能在 Sandbox 中修改任何文件，隔离确保不影响项目
- Tester 每轮都调用 OpenCode 检查并更新测试用例，确保测试覆盖随代码演进
- 生成的测试文件会回写，后续迭代 OpenCode 可直接修复和补充
- Analyzer 同理，防止 OpenCode 在分析时修改项目代码

### 3.5 设计文档驱动的开发模式

ASR 实际的输入是 **DESIGN.md**（必需）：
...
- **BuilderAgent（开发专家）**：`"根据 DESIGN.md 进行功能开发，包含基本的单元测试"`
- **TesterAgent（测试专家）**：`"读取 DESIGN.md → 检查已有测试 → 修复问题 → 补充新测试到 tests/ 目录"`
- **AnalyzerAgent（验收专家）**：`"读取 DESIGN.md → 对比代码 → 按缺失功能/逻辑错误/约束违反分类输出偏差，标注 [HIGH]/[MEDIUM]/[LOW]"`

Analyzer 输出为纯文本，直接传递给 Builder 作为修复依据。通过正则匹配 `[HIGH]`/`[CRITICAL]` 标记计算严重度计数，匹配 `ALL CLEAR` 判断规格一致性。

### 3.6 本地化 A2A 事件系统

ASR 采用**文件化 A2A 协议**，不引入 Kafka/NATS 等分布式中间件：

- **19 种事件类型**：覆盖任务创建、代码生成、测试、补丁、分析、收敛等完整生命周期
...
**系统要求**：
- Python 3.12.9（通过 pyenv 管理，详见项目 README.md）
- OpenCode CLI >= 1.15（需提前安装并配置 LLM 后端）
- oh-my-openagent >= 3.17（OpenCode 多智能体编排插件，推荐）
...
# 主要依赖：click, pydantic>=2.0, pyyaml, filelock, pytest, fastapi, httpx
```

**配置 LLM 后端**（通过 OpenCode 配置文件或环境变量）：

```bash
# OpenCode 配置 ~/.config/opencode/opencode.json
# 需配置 provider 和 model，详见 OpenCode 官方文档
# ASR 环境变量：
ASR_OPENCODE_TIMEOUT=14400      # OpenCode 调用超时（秒），默认 24400
ASR_VERBOSE=1                    # 启用详细日志（开发调试推荐）
ASR_STREAM_LIMIT=10485760        # 输出流缓冲区（字节），默认 10MB
```
...
最大迭代轮次: 10
  ...
  [第1轮] 代码生成  错误:0  🔧  | Token:12.3k/4.5k  | 补丁:0 文件:3 代码行:156 初始生成
  [第2轮] 测试验证  错误:3  ❌  | Token:8.1k/2.3k   | 通过:12/15 失败:test_foo,test_bar
  [第3轮] 代码修复  错误:3  🔧  | Token:5.2k/1.8k   | 修复3个失败
  [第4轮] 测试验证  错误:0  ✅  | Token:3.1k/1.2k   | 通过:15/15
  [第5轮] 规格分析  错误:0  ✅  | 规格:一致
...
| 依赖 | 版本要求 | 说明 |
|------|---------|------|
| Python | **3.12.9**（pyenv 管理） | 核心运行时 |
| OpenCode CLI | >= 1.15 | LLM 调用层（必需） |
| oh-my-openagent | >= 3.17（推荐） | 多智能体编排插件（Plan→Review→Execute） |
| pytest | >= 7.0 | 测试执行 |
| pydantic | >= 2.0 | 数据模型 |
| click | >= 8.0 | CLI 界面 |
| pyyaml | >= 6.0 | 配置/YAML 解析 |
| filelock | >= 3.0 | 原子写入 |
| fastapi | >= 0.115 | Web API（可选） |
| uvicorn | >= 0.30 | Web 服务器（可选） |
| httpx | >= 0.27 | HTTP 客户端 |
...
| 其他 OpenAI 兼容接口 | ✅ 兼容 | 通过配置环境变量 FEASIBILITY_LLM_API_BASE 即可 |

**模型选择建议**：
- 核心要求是**长上下文能力**（≥ 200K），因为 Builder 需要读 DESIGN.md + 已有代码
- Builder、Tester 和 Analyzer任务（代码生成/修复）建议使用参数量较大的模型（≥ 200B）
- 生产环境建议设置 `ASR_OPENCODE_TIMEOUT=14400`（4小时），避免复杂任务超时
...
│   │   ├── analyzer.py          # AnalyzerAgent（Sandbox + 纯文本分析）
...
- **输入**：13k token 设计文档（DESIGN.md，可研报告编译器 v4.3 设计方案）
- **输出**：完整可运行的代码工程
  - runtime/：核心运行时 8,343 行（15 文件）
  - packs/：Pack 配置体系（abc 元数据 + llm_corpus 领域 Pack）
  - tests/：自动化测试文件
- **迭代**：10 轮收敛循环，Builder 生成代码 → Tester 测试 → Analyzer 检查对齐
- **模型**：glm-4.7-fp8（200K 上下文，本地部署），oh-my-openagent 提供多智能体编排增强

### E. 端到端效果验证

详见独立文档：`效果验证测试报告_ASR系统.md`（对比 opencode CLI / ASR 系统 / 人工协作 longdoc 1.0 三方案）

---

*本报告对应系统版本：ASR v1.3 | 文档同步日期：2026-05-29*
